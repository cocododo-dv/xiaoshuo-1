...
- ``windows``:按原书顺序的样例窗——章号、位置(章首 / 章末 / 整章 / 章中)、字数与段数、是按哪条配额选进来的
  (``slot``)、占比最大的段落类型(``paragraph_type``),以及学习作业给这一窗打的场面 / 情绪 / 手法标签与一句话
  梗概(``gist``,专名已换成代称);
...
def scene_preview_payload(
    ...
    *,
        ...
        windows.append(
            {
                ...
                "devices": list(window_tags.get("devices") or ref.get("devices") or []),
    ...
    normalized = normalize_binding_config(None, config)
    scene_payload = None
    if scene_id:
    ...
    return {
        ...
        "sizes": {
            ...
            "card_lines": int(stats.get("positive_lines") or 0) + int(stats.get("forbidden_lines") or 0),
            "voice_lines": int(stats.get("voice_lines") or 0),
        },
        "notices": list(audit.get("notices") or []),
        "samples_blocked": audit.get("samples_blocked"),
        "legacy_profile": bool(audit.get("legacy_profile")),
    }
