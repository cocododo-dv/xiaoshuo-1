...
过去同一段文字有三套口径：``voice_signature`` 按全文汇总、``metrics.MetricsEngine`` 按「逐段算完再求平均」
（一句话的段和一百句话的段权重相同——真实参考书上问句密度 9.27 vs 汇总 4.8、句长标准差 7.45 vs 21.8）、
段落形状又是另一套；分段规则也各不相同（空行 / 单换行 / 整场一段），虚词表两套，「字数」三种定义。
本模块是唯一的测量入口，一遍算完：
...
# 旧 metrics 的文言 / 口语 / 比喻 / 拟人代理（名字随 MetricsEngine 保留；不进读数特征）
