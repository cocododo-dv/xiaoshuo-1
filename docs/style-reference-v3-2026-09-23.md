...
- 模型：opus 起草的首稿最像（85.4 / 88.5，都在范围内，flash 首稿 89.6–98.3），但这个中转的 opus 在结构化输出里间歇带乱码（4 次长稿
  2 次，同一场的首稿与修复都中，那一场失败；完整性门拦下、没有坏字进正文）。本客户端按严格 UTF-8 解析，乱码只可能来自中转一侧。
  **已处理**：回来的正文带 prompt 里没有的 U+FFFD 时，客户端在普通重试额度内原样重发（尝试行记 `LLM_RESPONSE_CORRUPTED_TEXT`、
  派发类型 `response_parse_retry`，照常记账）；最后一次仍带乱码就照常交回、由完整性门决定。代价是这个中转上的 opus 起草可能多花
  一两次调用（每次约 110 秒、6.5 万输入 tokens），换来不因乱码丢场。默认仍是作者选的 flash；想更像可以在系统配置里把 `style_draft`
  路由换成 opus。
...
- **最终验证（合并后的分支）**：后端全量（非 Chroma，四个分片）3,234 通过 / 5 跳过 / 0 失败；前端全量 1,147 通过、生产构建通过（并发跑
  全量时一个 5 秒超时的用例单独重跑通过——负载超时，不是真红）；React 契约 E2E 在独立端口（8011 / 5176）全部通过（风格页 8 条断言通过、
  「有书时四步」因夹具后端没有参考书而跳过）。
